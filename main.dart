...
void drawCell(Point coords, String color) {  
  	ctx..fillStyle = color..strokeStyle = "black";
...
void clear() {
	ctx..fillStyle = "black"..fillRect(0, 0, canvas.width, canvas.height);
}
...
class Snake {
	...
	static const int START_LENGTH = 7;
	List<Point> _body; // coordinates of body segments
  ...
  Point get tail => _body.last;
	...
	void _checkInput() {  
  	...
  	void _move() {
  		// Add new head segment and remove the old tail segment
  		grow();
  		_body.removeLast();
  	}

  	void _draw() {
  		// Draw each body segment from the head
